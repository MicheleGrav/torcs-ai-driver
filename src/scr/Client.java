...
public class Client {
	...
	private static int k; //per scegliere da terminale il numero di vicini
	...
	private static void parseParameters(String[] args) {
		...
		clientId = "SCR";
		verbose = false;
		k = 5;  //valore di default 1 per il KNN
		maxEpisodes = 1;
		maxSteps = 0;
		stage = Stage.UNKNOWN;
		trackName = "unknown";
		...
		for (int i = 1; i < args.length; i++) {
			...
			if (entity.equals("k")) {  //Parsing da stringa a intero
	...
	private static Controller load(String name) {
		...
		try {
			//Capisco il nome della classe passata da terminale
			Class<?> classe = Class.forName(name);
		
			//Definisco il costruttore della classe
			Constructor<?> costruttore = classe.getConstructor(int.class);
		
			//Creo un istanza della classe
			controller = (Controller) costruttore.newInstance(Client.k);
			...
			// Handle missing constructor exception
			e.printStackTrace();
